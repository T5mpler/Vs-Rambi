...
class PlayState extends MusicBeatState
{
	...
	public var boyfriends:Array<Boyfriend> = new Array<Boyfriend>();
	...
	public var rambi:Boyfriend;
	public var currentBoyfriend:Boyfriend;
	...
	override public function create()
	{
		...
		{
			case 'darkSpace':
				var bg:BGSprite = new BGSprite('him/DarkSpace', -1600, -800, 1, 1.1);
				bg.setGraphicSize(Std.int(bg.width * 4.5));
				bg.updateHitbox();
				add(bg);
			case 'farm':
				var bg:BGSprite = new BGSprite('rambi/bg', -500, -100, 1, 1);
		...
		currentBoyfriend = boyfriend;

		if (SONG.song.toLowerCase() == 'torture')
		{
			rambi = new Boyfriend(0, 0, 'rambi-playable');
			startCharacterPos(rambi);
			boyfriendGroup.add(rambi);
			boyfriends.push(rambi);
			startCharacterLua(rambi.curCharacter);
		}
		...
		iconP1 = new HealthIcon(SONG.song.toLowerCase() == 'torture' ? 'rambi-and-bf' : boyfriend.healthIcon, true);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (!isStoryMode)
			{
				...
				{
					FlxTween.tween(babyArrow, {y: babyArrow.y + 10, alpha: targetAlpha}, 1, {ease: FlxEase.circOut, startDelay: 0 + ((Conductor.crochet / 1000) * 4)* i});
				}
			}
			else
			{
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			var chars:Array<Character> = new Array<Character>();
			if (rambi != null)
			{
				chars = [boyfriend, rambi, gf, dad];
			}
			else
			{
	...
	public function moveCamera(isDad:Bool)
	{
		...
		{
			camFollow.set(currentBoyfriend.getMidpoint().x - 100, currentBoyfriend.getMidpoint().y - 100);
		
			camFollow.x -= currentBoyfriend.cameraPosition[0];
			camFollow.y += currentBoyfriend.cameraPosition[1];
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		var char:Character = currentBoyfriend;
		if(daNote.gfNote) {
			char = gf;
		}
	...
	function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
	{
		if (!currentBoyfriend.stunned)
		{
			...
			if(currentBoyfriend.hasMissAnimations) {
				currentBoyfriend.playAnim(singAnimations[Std.int(Math.abs(direction))] + 'miss', true);
			}
			vocals.volume = 0;
		}
	}
	...
	{
		var char:Character = currentBoyfriend;
		var curSectionIndex = Math.floor(curStep / 16);
		var curSection = SONG.notes[Math.floor(curStep / 16)];

		if ((note.strumTime >= getSectionStartTime(curSectionIndex) || note.strumTime <= getSectionStartTime(curSectionIndex + 1) ) && curSection.rambiSection && currentBoyfriend != rambi)
		{
			char = rambi;
		}
		if (!note.wasGoodHit)
		{
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					case 'Hurt Note': //Hurt note
						if(currentBoyfriend.animation.getByName('hurt') != null) {
							currentBoyfriend.playAnim('hurt', true);
							currentBoyfriend.specialAnim = true;
						}
				}
			...
			if (!note.noAnimation) {
				...
				//}else{
					...
					else if (note.noteType == 'Rambi Sings')
					{
						currentBoyfriend = rambi;
						rambi.playAnim(animToPlay + daAlt, true);
						rambi.holdTimer = 0;
					}
					else if (note.noteType != "Rambi & BF Sing") 
					{
	...
	override function stepHit()
	{
		...
		{
			if (SONG.notes[Math.floor(curStep / 16)].rambiSection)
			{
				currentBoyfriend = rambi;
			}
			else
	...
	override function beatHit()
	{
		...
		if (SONG.notes[Math.floor(curStep / 16)] != null)
		{
			...
			setOnLuas('rambiSection', SONG.notes[Math.floor(curStep / 16)].rambiSection);
			// else
			// Conductor.changeBPM(SONG.bpm);
		}
